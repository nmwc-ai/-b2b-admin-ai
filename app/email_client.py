"""Gmail IMAP — 수신(fetch_new_emails) / 초안 저장(create_draft) / 보낸메일(fetch_sent_emails).

발송은 하지 않는다 (사람이 Gmail 임시보관함에서 확인 후 발송).
서버리스에서 요청당 IMAP 연결을 연다.
"""
...
def fetch_new_emails() -> list:
    """B2B_INQUIRY 레이블의 UNSEEN 메일 목록 반환 (읽으면 Seen 처리)."""
...
def _find_folder_by_flag(mail, flag: str, default: str) -> str:
    _, folders = mail.list()
    for f in folders:
...
def _find_drafts_folder(mail) -> str:
...
def fetch_sent_emails(limit: int = 30, since_uid: str = None) -> list:
    ...
    try:
        ...
        for uid in uids:
            ...
            body_clean = _clean_reply_body(_get_body(msg))
...
def create_draft(to: str, subject: str, body: str) -> None:
    ...
    mail.append(drafts, '\\Draft', imaplib.Time2Internaldate(time.time()), msg.as_bytes())
    mail.logout()
